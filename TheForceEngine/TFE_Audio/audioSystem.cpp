...
static inline TFE_Mutex TFE_CreateMutex()  { InitializeCriticalSection(&s_critSection); return &s_critSection; }
static inline void TFE_DestroyMutex(TFE_Mutex) { DeleteCriticalSection(&s_critSection); }
static inline void TFE_LockMutex(TFE_Mutex m)  { EnterCriticalSection(m); }
static inline void TFE_UnlockMutex(TFE_Mutex m){ LeaveCriticalSection(m); }
#else
#include <SDL_mutex.h>
#include <algorithm>
#define XBOX_AUDIO_LOGF(...) ((void)0)
typedef SDL_mutex* TFE_Mutex;
static inline TFE_Mutex TFE_CreateMutex()       { return SDL_CreateMutex(); }
static inline void TFE_DestroyMutex(TFE_Mutex m){ SDL_DestroyMutex(m); }
static inline void TFE_LockMutex(TFE_Mutex m)   { SDL_LockMutex(m); }
static inline void TFE_UnlockMutex(TFE_Mutex m) { SDL_UnlockMutex(m); }
#endif
...
struct SoundSource
{
	...
	// Callback.
	SoundFinishedCallback finishedCallback;
...
namespace TFE_Audio
{
	...
	static AudioThreadCallback s_audioThreadCallback = NULL;
	...
	{
		XBOX_AUDIO_LOGF("Audio", "init begin null=%d outputId=%d", useNullDevice ? 1 : 0, outputId);
		...
		XBOX_AUDIO_LOGF("Audio", "settings volume=%.3f", soundSettings->soundFxVolume);
		...
		if (!audDev)
		{
			XBOX_AUDIO_LOGF("Audio", "device init failed");
		...
		s_nullDevice = false;
		XBOX_AUDIO_LOGF("Audio", "init complete sources=%u", s_sourceCount);
		return true;
	}

	void shutdown()
	{
		XBOX_AUDIO_LOGF("Audio", "shutdown null=%d", s_nullDevice ? 1 : 0);
	...
	void stopAllSounds()
	{
		...
		XBOX_AUDIO_LOGF("Audio", "stopAllSounds sourceCount=%u", s_sourceCount);
		TFE_LockMutex(s_mutex);
		s_sourceCount = 0u;
		memset(s_sources, 0, sizeof(SoundSource) * MAX_SOUND_SOURCES);
		for (s32 i = 0; i < MAX_SOUND_SOURCES; i++)
		{
		...
		TFE_UnlockMutex(s_mutex);
	}

	void selectDevice(s32 id)
	{
		XBOX_AUDIO_LOGF("Audio", "selectDevice requested=%d", id);
		if (id < 0)
		{
	...
	void setVolume(f32 volume)
	{
		...
		XBOX_AUDIO_LOGF("Audio", "setVolume %.3f", volume);
	}
	...
	{
		if (!buffer || s_nullDevice) { return NULL; }
	...
	static void audioCallback(void* userData, unsigned char* outputBuffer, int bufsize)
	{
		...
		for (u32 s = 0; s < s_sourceCount && !s_paused; s++, snd++)
		{
			...
			for (u32 i = 0; i < frames;)
			{
				...
				const u32 _minA = snd->sampleIndex + frames - i;
				const u32 end = sndBufferSize < _minA ? sndBufferSize : _minA;
		...
		for (u32 i = 0; i < frames; i++, buffer += 2)
		{
			...
			buffer[0] = valueLeft  < -c_channelLimit ? -c_channelLimit : (valueLeft  > c_channelLimit ? c_channelLimit : valueLeft);
			buffer[1] = valueRight < -c_channelLimit ? -c_channelLimit : (valueRight > c_channelLimit ? c_channelLimit : valueRight);
		...
		s_soundIterMaxF = s_soundIterMaxF > soundIterDeltaMS ? s_soundIterMaxF : soundIterDeltaMS;
