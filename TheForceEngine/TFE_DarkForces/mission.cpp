...
#define CHEAT_CMD(c) \
	...
	void handlePaletteFx()
	{
		...
		if (s_luminanceMask[0] || s_luminanceMask[1] || s_luminanceMask[2])
		{
			...
			lumMaskGpu.x = s_luminanceMask[0] ? 1.0f : 0.0f;
				lumMaskGpu.y = s_luminanceMask[1] ? 1.0f : 0.0f;
				lumMaskGpu.z = s_luminanceMask[2] ? 1.0f : 0.0f;
		}
		...
		if (s_screenFxEnabled)
		{
			if (s_healthFxLevel || s_shieldFxLevel || s_flashFxLevel)
			{
				...
				const f32 _hfx = f32(s_healthFxLevel)  / 63.0f;
				const f32 _sfx = f32(s_shieldFxLevel)  / 63.0f;
				const f32 _ffx = f32(s_flashFxLevel)   / 63.0f;
				palFxGpu.x = _hfx < 1.0f ? _hfx : 1.0f;
				palFxGpu.y = _sfx < 1.0f ? _sfx : 1.0f;
				palFxGpu.z = _ffx < 1.0f ? _ffx : 1.0f;
			}
			else if (s_palModified)
			{
				updateBasePal = JTRUE;
