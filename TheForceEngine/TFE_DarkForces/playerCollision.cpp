	...
	void console_exportTexture(const std::vector<std::string>& args)
	{
		...
		if (name)
		{
			...
			if (TFE_Paths::getFilePath(name, &path))
			{
				...
				file.readBuffer(&s_exportBuffer[0], (u32)file.getSize());
				file.close();
				...
				{
					file.writeBuffer(&s_exportBuffer[0], (u32)s_exportBuffer.size());
					file.close();
				}
			}
			...
			u32* outBuffer = (u32*)&s_exportBuffer[0];
			...
			outBuffer = (u32*)&s_exportBuffer[0];
