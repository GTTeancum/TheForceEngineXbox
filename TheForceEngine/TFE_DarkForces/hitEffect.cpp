...
namespace TFE_DarkForces
{
	...
	{
		// TFE: Effect data is now defined externally (effects.json).
	...
	EffectData setEffectData(HitEffectID type, TFE_ExternalData::ExternalEffect* extEffects)
	{
		EffectData r;
		r.type = type;
		r.spriteData = TFE_Sprite_Jedi::getWax(extEffects[type].wax, POOL_GAME);
		r.force = FIXED(extEffects[type].force);
		r.damage = FIXED(extEffects[type].damage);
		r.explosiveRange = FIXED(extEffects[type].explosiveRange);
		r.wakeupRange = FIXED(extEffects[type].wakeupRange);
		r.soundEffect = sound_load(extEffects[type].soundEffect, SoundPriority(extEffects[type].soundPriority));
		return r;
	}
