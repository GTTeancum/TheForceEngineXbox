...
namespace TFE_DarkForces
{
	...
	{
		{ WeaponAnimState _def; memset(&_def, 0, sizeof(_def)); SERIALIZE(SaveVersionInit, s_weaponAnimState, _def); }
	...
	void weapon_startup()
	{
		...
		// Gasmask texture: tolerate missing/empty external data gracefully
		// (e.g. if weapons.json failed to load on Xbox first boot).
		TFE_ExternalData::ExternalGasmask* gm = TFE_ExternalData::getExternalGasmask();
		if (gm && gm->texture && gm->texture[0])
		{
			s_gasmaskTexture = loadWeaponTexture(gm->texture);
			s_gasMaskXpos = gm->xPos;
			s_gasMaskYpos = gm->yPos;
		}
		else
		{
			s_gasmaskTexture = NULL;
			s_gasMaskXpos = 0;
			s_gasMaskYpos = 0;
		}
	...
	void weapon_handleOffAnimation(MessageType msg)
	{
		...
		s_weaponAnimState.frame = s_curPlayerWeapon->frame;
		s_weaponAnimState.startOffsetX = s_curPlayerWeapon->xOffset; s_weaponAnimState.startOffsetY = s_curPlayerWeapon->yOffset;
		s_weaponAnimState.xSpeed = 3; s_weaponAnimState.ySpeed = 4;
		s_weaponAnimState.frameCount = 10; s_weaponAnimState.ticksPerFrame = s_superCharge ? 1u : 2u;
	...
	void weapon_handleOnAnimation(MessageType msg)
	{
		...
		// s_prevWeapon is the weapon we are switching away from.
		s_weaponAnimState.frame = 0;
		s_weaponAnimState.startOffsetX = 30; s_weaponAnimState.startOffsetY = 40;
		s_weaponAnimState.xSpeed = -3; s_weaponAnimState.ySpeed = -4;
		s_weaponAnimState.frameCount = 10; s_weaponAnimState.ticksPerFrame = s_superCharge ? 1u : 2u;
		task_callTaskFunc(weapon_animateOnOrOffscreen);
		task_end;
		...
		while (1)
		{
			...
			else if (msg == MSG_HOLSTER)
			{
				...
				if (!s_weaponOffAnim)
				{
					...
					s_weaponAnimState.frame = s_curPlayerWeapon->frame;
					s_weaponAnimState.startOffsetX = 0; s_weaponAnimState.startOffsetY = 0;
					s_weaponAnimState.xSpeed = 3; s_weaponAnimState.ySpeed = 4;
					s_weaponAnimState.frameCount = 10; s_weaponAnimState.ticksPerFrame = s_superCharge ? 1u : 2u;
					task_callTaskFunc(weapon_animateOnOrOffscreen);
					s_weaponOffAnim = JTRUE;
