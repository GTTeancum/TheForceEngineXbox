...
namespace TFE_DarkForces
{
	...
	{
		SoundSourceId phase2aSndID;
		SoundSourceId phase2bSndID;
		SoundSourceId phase2cSndID;
		SoundSourceId phase2RocketSndID;
		s32 trooperNum;
	};
	...
	void phaseTwo_handleExplosion(MessageType msg)
	{
		...
		if (local(physicsActor)->alive)
		{
			task_localBlockBegin;
				...
				vec3_fixed pos; pos.x = local(obj)->posWS.x; pos.y = local(obj)->posWS.y - local(obj)->worldHeight; pos.z = local(obj)->posWS.z;
				vec3_fixed pushDir;

				computeExplosionPushDir(&pos, &pushDir);
				local(vel).x = mul16(pushDir.x, force); local(vel).y = mul16(pushDir.y, force); local(vel).z = mul16(pushDir.z, force);
			task_localBlockEnd;
			...
			else
			{
				...
				local(physicsActor)->vel.x = local(vel).x >> 1; local(physicsActor)->vel.y = local(vel).y >> 1; local(physicsActor)->vel.z = local(vel).z >> 1;
	...
	void phaseTwo_handleDying(MessageType msg)
	{
		...
		task_localBlockBegin;
			...
			if (sector->secHeight - 1 < 0)
			{
				...
				collisionInfo.obj = item;
				collisionInfo.offsetX = FIXED(3);
				collisionInfo.offsetY = 0;
				collisionInfo.offsetZ = FIXED(3);
				collisionInfo.stepUpHeight = ONE_16;
				collisionInfo.stepDownHeight = COL_INFINITY;
				collisionInfo.height = ONE_16;
				collisionInfo.width = item->worldWidth;
	...
	void phaseTwo_serialize(Logic*& logic, SecObject* obj, Stream* stream)
	{
		...
		SERIALIZE_ZERO(SaveVersionInit, physicsActor->vel);
		SERIALIZE_ZERO(SaveVersionInit, physicsActor->lastPlayerPos);
