...
namespace TFE_DarkForces
{
	...
	Tick exploderMsgFunc(s32 msg, ActorModule* module, MovementModule* moveMod)
	{
		...
		if (msg == MSG_DAMAGE)
		{
			if (damageMod->hp > 0)
			{
				...
				else
				{
					...
					memset(&proj->vel, 0, sizeof(proj->vel));
		...
		else if (msg == MSG_EXPLOSION)
		{
			...
			vec3_fixed pushDir;
			vec3_fixed pos; pos.x = obj->posWS.x; pos.y = obj->posWS.y - obj->worldHeight; pos.z = obj->posWS.z;
			...
			memset(&proj2->vel, 0, sizeof(proj2->vel));
			...
			CollisionInfo colInfo; memset(&colInfo, 0, sizeof(colInfo));
