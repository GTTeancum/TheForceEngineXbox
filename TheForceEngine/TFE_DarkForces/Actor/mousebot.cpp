...
namespace TFE_DarkForces
{
	...
	static MouseBotResources s_mouseBotRes;
	...
	MessageType mousebot_handleExplosion(MessageType msg, MouseBot* mouseBot)
	{
		...
		if (mouseBot->actor.alive)
		{
			...
			{
				phyActor->vel.x = vel.x >> 1; phyActor->vel.y = vel.y >> 1; phyActor->vel.z = vel.z >> 1;
				sound_playCued(s_mouseBotRes.sound1, obj->posWS);
			}
		}
		return msg;
	...
	void mousebot_exit()
	{
		memset(&(s_mouseBotRes), 0, sizeof(s_mouseBotRes));
	}
	...
	void mousebot_serialize(Logic*& logic, SecObject* obj, Stream* stream)
	{
		...
		SERIALIZE_ZERO(SaveVersionInit, physActor->vel);
		SERIALIZE_ZERO(SaveVersionInit, physActor->lastPlayerPos);
