...
namespace TFE_DarkForces
{
	...
	struct DragonShared
	{
		SoundSourceId kellSound0;
		SoundSourceId kellSound1;
		SoundSourceId kellSound2;
		SoundSourceId kellSound3;
		SoundSourceId kellSound4;
		s32 dragonNum;
	};
	...
	void kellDragon_handleExplosion(MessageType msg)
	{
		...
		if (local(physicsActor)->alive)
		{
			task_localBlockBegin;
				...
				local(vel).x = mul16(force, pushVel.x); local(vel).y = mul16(force, pushVel.y); local(vel).z = mul16(force, pushVel.z);
			task_localBlockEnd;
	...
	void kellDragon_lookForPlayer(MessageType msg)
	{
		...
		while (local(physicsActor)->state == DRAGONSTATE_SEARCH)
		{
			...
			if (local(arrived))
			{
				...
				{
					target.x = s_eyePos.x; target.z = s_eyePos.z;
				}
