...
namespace TFE_DarkForces
{
	...
	void player_init()
	{
		...
		memset(&s_playerInfo, 0, sizeof(s_playerInfo)); // Make sure this is clear...
	...
	void player_setupObject(SecObject* obj)
	{
		...
		memset(&(s_playerLogic.move), 0, sizeof(s_playerLogic.move));
		memset(&(s_playerLogic.dir), 0, sizeof(s_playerLogic.dir));
	...
	void setCameraOffset(fixed16_16 offsetX, fixed16_16 offsetY, fixed16_16 offsetZ)
	{
		s_camOffset.x = offsetX;
		s_camOffset.y = offsetY;
		s_camOffset.z = offsetZ;
	}
	...
	void player_warp(const ConsoleArgList& args)
	{
		...
		if (sector)
		{
			s_playerObject->posWS.x = x;
			s_playerObject->posWS.y = y;
			s_playerObject->posWS.z = z;
	...
	void playerLogic_serialize(Logic*& logic, SecObject* obj, Stream* stream)
	{
		...
		SERIALIZE_ZERO(ObjState_InitVersion, s_playerInfo);
