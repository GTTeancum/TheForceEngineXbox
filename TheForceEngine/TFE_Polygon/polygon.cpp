...
namespace TFE_Polygon
{
	...
	void addTriangle(s32 i0, s32 i1, s32 i2)
	{
		...
		else
		{
			...
			{ Triangle t; memset(&t, 0, sizeof(t)); s_triangles.push_back(t); }
			tri = &s_triangles[id];
			tri->id = id;
		}
	...
	void addEdge(s32 i0, s32 i1)
	{
		...
		{ TriEdge te; te.idx[0] = i0; te.idx[1] = i1; te.refCount = 0; s_edges.push_back(te); }
	}
		...
		for (s32 e = 0; e < edgeCount; e++, edge++)
		{
			...
			if (dz != 0)
			{
				...
				else if (p.z != z0)
				{
					if (p.z != z1)
					{
						Vec2f seg0; seg0.x = x0; seg0.z = z0;
						Vec2f seg1; seg1.x = x1; seg1.z = z1;
						PointSegSide side = lineSegmentSide(p, seg0, seg1);
						if (side == PS_OUTSIDE)
						{
							crossings++;
						}
			...
			{
				Vec2f s0; s0.x = x0; s0.z = z0;
				Vec2f s1; s1.x = x1; s1.z = z1;
				if (lineSegmentSide(p, s0, s1) == PS_ON_LINE)
				{
					return true;
				}
			}
	...
	bool computeTriangulation(Polygon* poly, u32 debug)
	{
		...
		{
			{ Vec2f pt; pt.x = vtx->x - s_coordCenter.x; pt.z = vtx->z - s_coordCenter.z; addPoint(pt); }
		}
		...
		for (size_t t = 0; t < triCount; t++, tri++)
		{
			...
			if (!(debug & PDBG_SHOW_SUPERTRI) && !(debug & PDBG_SKIP_INSIDE_TEST))
			{
				...
				Vec2f cp; cp.x = tri->centroid.x + s_coordCenter.x; cp.z = tri->centroid.z + s_coordCenter.z;
				if (!pointInsidePolygon(poly, cp))
				{
					deleteTriangle(tri);
					continue;
				}
		...
		const Vec2f* srcVtx = &s_vertices[0];
		Vec2f* dstVtx = &poly->triVtx[0];
		for (size_t v = 0; v < finalVtxCount; v++, srcVtx++)
		{
			dstVtx[v].x = srcVtx->x + s_coordCenter.x; dstVtx[v].z = srcVtx->z + s_coordCenter.z;
		}
	...
	bool buildContour(const BEdge* startEdge, const std::vector<BEdge>& edgesList, std::vector<s32>& remEdges, Contour* outContour)
	{
		...
		const f32 area = TFE_Polygon::signedArea((s32)outContour->vtx.size(), &outContour->vtx[0]);
	...
	void buildPathsFromContours(const std::vector<Contour>& contourList, ClipperLib::Paths* paths)
	{
		...
		for (s32 i = 0; i < contourCount; i++, contour++, path++)
		{
			...
			for (s32 v = 0; v < vtxCount; v++)
			{
				...
				// If not than push it.
				path->push_back(ClipperLib::IntPoint(ClipperLib::cInt((f64)vtx[v].x * precision), ClipperLib::cInt((f64)vtx[v].z * precision)));
			}
		}
	}
	...
	void buildPathFromShape(const std::vector<Vec2f>& shape, ClipperLib::Path& outPath)
	{
		...
		const Vec2f* vtx = &shape[0];

		outPath.resize(shape.size());
		ClipperLib::IntPoint* outVtx = &outPath[0];
	...
	void buildContoursFromPaths(const ClipperLib::Paths& paths, s32 outsideClipCount, std::vector<Contour>* contourList)
	{
		...
		const ClipperLib::Path* path = &paths[0];

		contourList->resize(contourCount);
		Contour* contour = &(*contourList)[0];
		for (s32 i = 0; i < contourCount; i++, contour++, path++)
		{
			const s32 vtxCount = (s32)path->size();
			const ClipperLib::IntPoint* srcVtx = &(*path)[0];

			contour->vtx.resize(vtxCount);
			Vec2f* dstVtx = &contour->vtx[0];
			...
			// Next determine the winding order.
			const f32 area = TFE_Polygon::signedArea(vtxCount, &contour->vtx[0]);
	...
	void buildShapeFromPath(const ClipperLib::Path& path, std::vector<Vec2f>& outShape)
	{
		const f64 precision = c_fromFixed;
		const s32 vtxCount = (s32)path.size();
		const ClipperLib::IntPoint* vtx = &path[0];

		outShape.resize(vtxCount);
		Vec2f* outVtx = &outShape[0];
	...
	void buildContoursFromPolygon(const BPolygon* poly, std::vector<Contour>* contours)
	{
		...
		while (!remEdges.empty())
		{
			Contour contour; contour.clockwise = true; contour.outsideClip = false;
	...
	{
		BPolygon tmp; tmp.outsideClipRegion = false;

		const s32 subjEdgeCount = (s32)subject->edges.size();
		const BEdge* sEdge = &subject->edges[0];
		...
		for (s32 eS = 0; eS < subjEdgeCount; eS++, sEdge++)
		{
			...
			const BEdge* cEdge = &clip->edges[0];
			for (s32 eC = 0; eC < clipEdgeCount; eC++, cEdge++)
			{
				...
				if (lineSegmentsIntersect(sEdge->v0, sEdge->v1, cEdge->v0, cEdge->v1, &vI, &u, &v))
				{
					...
					if (u > 0.0f && u < 1.0f && !TFE_Polygon::vtxEqual(&sEdge->v0, &vI) && !TFE_Polygon::vtxEqual(&sEdge->v1, &vI))
					{
						...
						{ Intersection is; is.vI = vI; is.u = u; is.v = v; is.eS = eS; is.eC = eC; is.id = itrId; interSrc.push_back(is); }
						itrId++;
					}
				}
			}
	...
	void addPointToList(const Vec2f pt, std::vector<Vec2f>* insertionPt)
	{
		...
		const Vec2f* srcPt = &(*insertionPt)[0];
	...
	void insertPointsIntoPolygons(const std::vector<Vec2f>& insertionPt, std::vector<BPolygon>* poly)
	{
		...
		const Vec2f* insertVtx = &insertionPt[0];
		const s32 polyCount = (s32)poly->size();
		BPolygon* dstPoly = &(*poly)[0];
		for (s32 p = 0; p < polyCount; p++, dstPoly++)
		{
			...
			const BEdge* sEdge = &dstPoly->edges[0];
			for (s32 eS = 0; eS < subjEdgeCount; eS++, sEdge++)
			{
				interSrc.clear();
				...
				for (s32 i = 0; i < insertionCount; i++)
				{
					...
					if (distSq < eps)
					{
						{ Intersection is; is.vI = insertVtx[i]; is.u = u; is.v = 0.0f; is.eS = eS; is.eC = 0; is.id = 0; interSrc.push_back(is); }
					}
				}
	...
	void clipPolygons(const BPolygon* subject, const BPolygon* clip, std::vector<BPolygon>& outPoly, BoolMode boolMode)
	{
		...
		for (s32 i = 0; i < outCount; i++, contour++)
		{
			...
			BPolygon poly; poly.outsideClipRegion = false;
		...
		for (s32 i = 0; i < outCount; i++, contour++)
		{
			...
			BPolygon* outerPoly = &firstPassPoly[0];
			...
			BEdge* edge = &parent->edges[0] + startIndex;
		...
		if (boolMode == BMODE_MERGE)
		{
			...
			BPolygon tmpPoly; tmpPoly.outsideClipRegion = false;
			BPolygon* srcPoly = &outPoly[0];
			for (s32 i = 0; i < outerPolyCount; i++)
			{
				...
				for (s32 j = 0; j < outerPolyCount; j++)
				{
					...
					const BEdge* sEdge = &srcPoly[i].edges[0];
					for (s32 eS = 0; eS < subjEdgeCount; eS++, sEdge++)
					{
						interSrc.clear();
						s32 itrId = 0;
						...
						const BEdge* cEdge = &srcPoly[j].edges[0];
						for (s32 eC = 0; eC < clipEdgeCount; eC++, cEdge++)
						{
							Vec2f vI;
							f32 u, v;
