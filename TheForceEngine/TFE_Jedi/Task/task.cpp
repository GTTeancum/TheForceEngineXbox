...
#ifdef _XBOX
// VC71 doesn't support variadic macros - use a no-op inline function.
static inline void TASK_MSG(const char* fmt, ...) { (void)fmt; }
#else
#ifdef TASK_DEBUG
...
namespace TFE_Jedi
{
	...
	void createRootTask()
	{
		...
		memset(&s_rootTask, 0, sizeof(s_rootTask));
