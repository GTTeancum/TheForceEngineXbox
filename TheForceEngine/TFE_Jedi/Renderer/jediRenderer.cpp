...
namespace TFE_Jedi
{
	...
	JBool render_setResolution(bool forceTextureUpdate)
	{
		...
		else
		{
			...
			{
				TFE_Sectors_GPU* sectorRendererGpu = (TFE_Sectors_GPU*)renderer_getSectorRenderer(TSR_CLASSIC_GPU);
				screenGPU_setHudTextureCallbacks((s32)s_hudTextureCallbacks.size(), &s_hudTextureCallbacks[0], updateTexturePacking);
			...
			{
				screenGPU_setHudTextureCallbacks((s32)s_hudTextureCallbacks.size(), &s_hudTextureCallbacks[0], true);
			}
	...
	JBool setSubRenderer(TFE_SubRenderer subRenderer/* = TSR_CLASSIC_FIXED*/)
	{
		...
		switch (subRenderer)
		{
			...
			{
				TFE_Sectors_GPU* sectorRendererGpu = (TFE_Sectors_GPU*)renderer_getSectorRenderer(TSR_CLASSIC_GPU);
				screenGPU_setHudTextureCallbacks((s32)s_hudTextureCallbacks.size(), &s_hudTextureCallbacks[0]);
				s_sectorRenderer = sectorRendererGpu;
