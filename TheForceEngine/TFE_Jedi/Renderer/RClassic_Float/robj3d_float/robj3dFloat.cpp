...
namespace RClassic_Float
{
	...
	void robj3d_draw(SecObject* obj, JediModel* model)
	{
		...
		if (model->flags & MFLAG_DRAW_VERTICES)
		{
			...
			robj3d_drawVertices(model->vertexCount, &s_verticesVS[0], model->polygons[0].color, scale);
			return;
		}
		...
		qsort(&s_visPolygons[0], visPolygonCount, sizeof(JmPolygon*), polygonSort);

		// Draw polygons
		JmPolygon** visPolygon = &s_visPolygons[0];
		JBool drawn = JFALSE;
