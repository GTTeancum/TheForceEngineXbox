...
namespace RClassic_Fixed
{
	...
	void robj3d_transformAndLight(SecObject* obj, JediModel* model)
	{
		...
		robj3d_transformVertices(model->vertexCount, (vec3_fixed*)model->vertices, xform, &offsetVS, &s_verticesVS[0]);
		...
		robj3d_transformVertices(model->polygonCount, (vec3_fixed*)model->polygonNormals, xform, &offsetVS, &s_polygonNormalsVS[0]);

		// Lighting
		if (model->flags & MFLAG_VERTEX_LIT)
		{
			robj3d_transformVertices(model->vertexCount, (vec3_fixed*)model->vertexNormals, xform, &offsetVS, &s_vertexNormalsVS[0]);
			robj3d_shadeVertices(model->vertexCount, &s_vertexIntensity[0], &s_verticesVS[0], &s_vertexNormalsVS[0]);
		}
	}
