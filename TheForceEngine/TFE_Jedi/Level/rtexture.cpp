...
namespace TFE_Jedi
{
...
#ifdef _XBOX
	typedef std::map<std::string, s32> TextureTable;
#else
	...
	struct TextureState
	{
		Allocator*    textureAnimAlloc;
		Task*         textureAnimTask;
	...
	{
		memset(&(s_texState), 0, sizeof(s_texState));
	...
	bool bitmap_getTextureIndex(TextureData* tex, s32* index, AssetPool* pool)
	{
		for (s32 p = 0; p < POOL_COUNT; p++)
		{
			...
			const LevelTexture* srcList = &s_textureList[p][0];
	...
	void bitmap_serializeLevelTextures(Stream* stream)
	{
		...
		list = &s_textureList[POOL_LEVEL][0];
	...
	TextureData** bitmap_getTextures(s32* textureCount, AssetPool pool)
	{
		...
		TextureData** list = &s_tempTextureList[0];
		const LevelTexture* srcList = &s_textureList[pool][0];
		for (s32 i = 0; i < count; i++)
		{
	...
	void bitmap_loadHD(const char* name, TextureData* texData, s32 scaleFactor, AssetPool pool)
	{
		...
		file.readBuffer(&s_buffer[0], (u32)size);
		file.close();
	...
	bool isAssetCustom(const char* archiveName)
	{
		...
		const std::string* names = &s_coreAchiveNames[0];
		for (s32 i = 0; i < count; i++)
		{
	...
	TextureData* bitmap_load(const char* name, u32 decompress, AssetPool pool, bool addToCache)
	{
		...
		if (addToCache)
		{
			...
			{ LevelTexture lt; lt.name = name; lt.texture = texture; s_textureList[pool].push_back(lt); }
