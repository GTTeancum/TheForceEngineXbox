...
struct TextureData
{
	...
	u8 flags;
	u8 compressed;  // 0 = not compressed, 1 = compressed (RLE), 2 = compressed (RLE0)
	u8 palIndex;    // TFE - which palette to use.
	u8 pad3;
	...
	// HD Texture replacements.
	s32 scaleFactor;				// Fill with scale factor.
	u8* hdAssetData;				// True color asset data, size = width * height * scaleFactor * scaleFactor
#endif
};
#pragma pack(pop)
