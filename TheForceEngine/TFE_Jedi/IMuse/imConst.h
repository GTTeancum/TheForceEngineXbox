...
//
// VC71 doesn't support variadic macros. On Xbox, replace each with an inline
// variadic function. The "enabled" variants still go to no-op since shipping
// iMuse traces are not necessary for the Xbox port.
#ifdef _XBOX
static inline void IM_DBG_MSG(const char* fmt, ...) { (void)fmt; }
static inline void IM_LOG_ERR(const char* fmt, ...) { (void)fmt; }
static inline void IM_LOG_MSG(const char* fmt, ...) { (void)fmt; }
static inline void IM_LOG_WRN(const char* fmt, ...) { (void)fmt; }
#else
