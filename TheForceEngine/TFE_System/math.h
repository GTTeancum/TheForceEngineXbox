...
namespace TFE_Math
{
	...
	inline Vec3f normalize(const Vec3f* vec)
	{
		...
		if (lenSq > FLT_EPSILON)
		{
			...
			nrm.x = vec->x * scale; nrm.y = vec->y * scale; nrm.z = vec->z * scale;
		}
		else
		{
			nrm = *vec;
	...
	inline Vec2f normalize(const Vec2f* vec)
	{
		...
		{
			const f32 scale = 1.0f / sqrtf(lenSq);
			nrm.x = vec->x * scale; nrm.z = vec->z * scale;
		}
		else
		{
			nrm = *vec;
	...
	{
		Vec3f r; r.x = a->y*b->z - a->z*b->y; r.y = a->z*b->x - a->x*b->z; r.z = a->x*b->y - a->y*b->x; return r;
	}
