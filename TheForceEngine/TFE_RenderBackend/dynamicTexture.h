...
{
public:
	DynamicTexture() : m_bufferCount(0), m_readBuffer(0), m_writeBuffer(0),
	                   m_width(0), m_height(0), m_format(DTEX_RGBA8),
	                   m_textures(NULL), m_stagingBuffers(NULL) {}
	~DynamicTexture();
	...
	inline const TextureGpu* getTexture() const
	{
		if (!m_textures) return NULL;
		return m_textures[m_readBuffer];
	}
