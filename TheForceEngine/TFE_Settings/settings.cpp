...
namespace TFE_Settings
{
	...
	{
		// Clear out game settings via constructor (C++03 compatible).
		s_gameSettings = TFE_Settings_Game();
...
#ifdef _XBOX
		// Xbox: skip all settings.ini disk I/O. main_xbox.cpp overrides
		// every important setting (window/graphics/renderer/colorMode)
		// immediately after init, so reading or writing settings.ini is
		// wasted work that only exposes us to FATX path-resolution hangs.
		// Defaults set above are sufficient until main_xbox.cpp runs.
		firstRun = true;
		return true;
#else
	...
	{
#ifdef _XBOX
		// Xbox: source data path is set explicitly by setupSourceDataPath()
		// in main_xbox.cpp (D:\DARK\). Auto-detect probes pathological
		// paths like "D:\Games/Dark Forces/DARK.GOB" which hang on Xbox
		// FATX (missing intermediate dirs - see xquake gold standard:
		// only probe under registered, validated search dirs).
		return;
#else
...
#ifdef _XBOX
			char* iniBuffer = (char*)malloc(len + 1);
			if (!iniBuffer) { settings.close(); return false; }
			iniBuffer[len] = 0;
			settings.readBuffer(iniBuffer, (u32)len);
			settings.close();
			parseIniFile(iniBuffer, len);
			autodetectGamePaths();
			free(iniBuffer);
#else
			s_iniBuffer.resize(len + 1);
			s_iniBuffer[len] = 0;
				settings.readBuffer(&s_iniBuffer[0], (u32)len);
			settings.close();
				parseIniFile(&s_iniBuffer[0], len);
			autodetectGamePaths();
#endif
			return true;
		}
	...
	void parseDark_ForcesSettings(const char* key, const char* value)
	{
		...
		else if (strcasecmp("airControl", key) == 0)
		{
			...
			if (v < 0) v = 0;
			if (v > 8) v = 8;
			s_gameSettings.df_airControl = v;
		}
	...
	bool isHdAssetValid(const char* assetName, HdAssetType type)
	{
		...
		const ModHdIgnoreList* list = &s_modSettings.ignoreList[0];
		for (size_t l = 0; l < listCount; l++)
		{
			...
			const std::string* ignoreList = NULL;
	...
	bool isHdAssetValid(const char* /*assetName*/, HdAssetType /*type*/)
	{
		return true; // No mod HD assets on Xbox.
	}
#endif // _XBOX
