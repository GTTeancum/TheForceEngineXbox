...
namespace TFE_ExternalData
{
	...
	void loadExternalProjectiles()
	{
...
#ifdef _XBOX
		// Xbox: keep JSONs alongside the GOBs in DARK\ so the user only has
		// one game-data folder to manage on the Xbox HDD.
		sprintf(extDataFile, "%sprojectiles.json", TFE_Paths::getPath(PATH_SOURCE_DATA));
#else
		...
		TFE_System::logWrite(LOG_MSG, "EXTERNAL_DATA", "Loading projectile data from: %s", extDataFile);
		FileStream file;
		if (!file.open(extDataFile, FileStream::MODE_READ))
		{
			TFE_System::logWrite(LOG_ERROR, "EXTERNAL_DATA", "Cannot open %s", extDataFile);
			return;
		}
	...
	void loadExternalEffects()
	{
		char extDataFile[TFE_MAX_PATH];
#ifdef _XBOX
		sprintf(extDataFile, "%seffects.json", TFE_Paths::getPath(PATH_SOURCE_DATA));
#else
		...
		TFE_System::logWrite(LOG_MSG, "EXTERNAL_DATA", "Loading effects data from: %s", extDataFile);
		FileStream file;
		if (!file.open(extDataFile, FileStream::MODE_READ))
		{
			TFE_System::logWrite(LOG_ERROR, "EXTERNAL_DATA", "Cannot open %s", extDataFile);
	...
	void loadExternalWeapons()
	{
		char extDataFile[TFE_MAX_PATH];
#ifdef _XBOX
		sprintf(extDataFile, "%sweapons.json", TFE_Paths::getPath(PATH_SOURCE_DATA));
#else
		...
		TFE_System::logWrite(LOG_MSG, "EXTERNAL_DATA", "Loading weapon data from: %s", extDataFile);
		FileStream file;
		if (!file.open(extDataFile, FileStream::MODE_READ))
		{
			TFE_System::logWrite(LOG_ERROR, "EXTERNAL_DATA", "Cannot open %s", extDataFile);
