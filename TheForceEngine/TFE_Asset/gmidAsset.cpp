...
#define DARK_FORCES_SYSEX 0x7D
	...
	GMidiAsset* get(const char* name)
	{
		...
		gmidAsset.readBuffer(&s_buffer[0], (u32)size);
		gmidAsset.close();
	...
	void addMidiEvent(Track* track, u32 tick, u8 evt, s8 channel, u8 data1, u8 data2)
	{
		...
		{ MidiTrackEvent ev = { tick, MTK_MIDI, index }; track->eventList.push_back(ev); }
		{ MidiEvent ev; ev.type = evt; ev.channel = channel; ev.data[0] = data1; ev.data[1] = data2; track->midiEvents.push_back(ev); }
	}

	void addTempoEvent(Track* track, u32 tick, f32 msPerTick)
	{
		const u32 index = (u32)track->tempoEvents.size();
		{ MidiTrackEvent ev = { tick, MTK_TEMPO, index }; track->eventList.push_back(ev); }
		{ MidiTempoEvent ev = { msPerTick }; track->tempoEvents.push_back(ev); }
	}

	void addMidiMarker(Track* track, u32 tick, const char* text)
	{
		const u32 index = (u32)track->markers.size();
		{ MidiTrackEvent ev = { tick, MTK_MARKER, index }; track->eventList.push_back(ev); }
		{ MidiMarker ev; memset(&ev, 0, sizeof(ev)); track->markers.push_back(ev); }
