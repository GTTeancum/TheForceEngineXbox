...
namespace TFE_Sprite
{
	...
	Sprite* getSprite(const char* name)
	{
		...
		const s32* frameIndex = &s_tempFrames[0];
		...
		const s32* cellIndex = &s_tempCells[0];
		...
		sprite->rect[2] = -32767;
		sprite->rect[3] = -32767;
		const u32* offsets = &cellImgPtr[0];
		for (s32 i = 0; i < totalFrameCount; i++)
		{
