	...
	bool parseVue(VueAsset* vue)
	{
		...
		while (bufferPos < len)
		{
			...
			else if (strcasecmp("transform", tokens[0].c_str()) == 0)
			{
				...
				{ VueTransform vt; vt.rotScale = rotScale; vt.translation = translation; transforms.push_back(vt); }
				transformIndex++;
			}
		}
		...
		memcpy(vue->transforms, &transforms[0], sizeof(VueTransform)*transforms.size());
		...
		{
			{ Vec3f t; t.x = vue->transforms[i].translation.x; t.y = -vue->transforms[i].translation.z; t.z = -vue->transforms[i].translation.y; vue->transforms[i].translation = t; }

			const Mat3 matVue = vue->transforms[i].rotScale;
			vue->transforms[i].rotScale.m0.x = matVue.m0.x; vue->transforms[i].rotScale.m0.y = matVue.m2.x; vue->transforms[i].rotScale.m0.z = matVue.m1.x;
			vue->transforms[i].rotScale.m1.x = -matVue.m0.z; vue->transforms[i].rotScale.m1.y = -matVue.m2.z; vue->transforms[i].rotScale.m1.z = -matVue.m1.z;
			vue->transforms[i].rotScale.m2.x = matVue.m0.y; vue->transforms[i].rotScale.m2.y = matVue.m2.y; vue->transforms[i].rotScale.m2.z = matVue.m1.y;
		}
