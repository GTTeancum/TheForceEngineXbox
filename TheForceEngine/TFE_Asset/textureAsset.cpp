...
namespace TFE_Texture
{
	...
	Texture* get(const char* name)
	{
		...
		const BM_Header* header = (BM_Header*)&s_buffer[0];
		const u8* data = &s_buffer[0] + sizeof(BM_Header);
...
#pragma pack(pop)
	...
	Texture* getFromDelt(const char* name, const char* archivePath)
	{
		...
		// Then read out the data.
		DeltHeader header = *((DeltHeader*)&s_buffer[0]);
		header.sizeX++;
		header.sizeY++;
		...
		loadDeltIntoFrame(&texture->frames[0], &imageDst, &s_buffer[0], (u32)s_buffer.size());
	...
	Texture* getFromAnim(const char* name, const char* archivePath)
	{
		...
		const s16 frameCount = *((s16*)&s_buffer[0]);
		const u8* frames = &s_buffer[0] + 2;
