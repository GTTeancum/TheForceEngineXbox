...
namespace TFE_Sprite_Jedi
{
	...
	bool loadFrameHd(const char* name, const JediFrame* frame, AssetPool pool, HdWax* hdWax, const WaxCell* cell)
	{
		...
		file.readBuffer(&s_buffer[0], (u32)fileSize);
		file.close();
	...
	JediFrame* getFrame(const char* name, AssetPool pool)
	{
		...
		s_buffer.resize(len);
		file.readBuffer(&s_buffer[0], u32(len));
		file.close();
	...
	bool isUniqueCell(u32 offset)
	{
		...
		const u32* offsetList = &s_cellOffsets[0];
	...
	void sprite_serializeSpritesAndFrames(Stream* stream)
	{
		...
		std::string* frameNames  = &s_frameNames[POOL_LEVEL][0];
		std::string* spriteNames = &s_spriteNames[POOL_LEVEL][0];
		std::string name;
	...
	void freePool(AssetPool pool)
	{
		...
		JediFrame** frameList = &s_frameList[pool][0];
		...
		JediWax** waxList = &s_spriteList[pool][0];
		...
		HdWax** hdWaxList = &s_hdSpriteList[pool][0];
	...
	bool getWaxIndex(JediWax* wax, s32* index, AssetPool* pool)
	{
		for (s32 p = 0; p < POOL_COUNT; p++)
		{
			...
			JediWax** waxList = &s_spriteList[p][0];
			for (size_t i = 0; i < waxCount; i++)
			{
				if (waxList[i] == wax)
				{
	...
	bool getFrameIndex(JediFrame* frame, s32* index, AssetPool* pool)
	{
		for (s32 p = 0; p < POOL_COUNT; p++)
		{
			...
			JediFrame** frameList = &s_frameList[p][0];
			for (size_t i = 0; i < frameCount; i++)
			{
