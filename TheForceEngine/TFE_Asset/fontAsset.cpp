...
namespace TFE_Font
{
	...
	FontTFE* get(const char* name)
	{
		...
		data = &s_buffer[0] + sizeof(FNT_Header);
		u32 offset = 0;
		font->maxWidth = 0;
		for (s32 i = 0; i < (s32)font->charCount; i++)
		{
...
#pragma pack(pop)
	...
	FontTFE* getFromFont(const char* name, const char* archivePath)
	{
		...
		FontHeader* header = (FontHeader*)&s_buffer[0];
		...
		const u8* data = &s_buffer[0] + sizeof(FontHeader);
		u32 pixelCount = 0u;
		...
		u32 imageOffset = 0u;
		const u8* srcBitmap = &s_buffer[0] + sizeof(FontHeader) + header->last;
